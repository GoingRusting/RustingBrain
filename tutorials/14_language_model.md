...
between. It is the number worth watching, because it has a meaning.
...
probabilities, append it, repeat.

Done naively, that is quadratic. To produce token 500 you would re-run all 499
previous tokens through all eight layers, and you have already done that work
499 times.

The **KV cache** keeps it:
...
for _ in 0..200 {
    let next = sample(logits.row(logits.rows - 1), 0.8, 40);
    generated.push(next);
...
`logits.row(logits.rows - 1)` is one score per vocabulary entry for the next
position. Turning that into a token is a choice:

**Greedy** — always the highest. Deterministic, and it loops: "the the the".

**Temperature** — divide the logits by `T` before softmax. `T < 1` sharpens
toward the top choice, `T > 1` flattens toward uniform. `0.7–0.9` is the usual
range; `T = 0` is greedy.

**Top-k** — keep the `k` highest and sample among those. The point is to cut
...
one wrong token derails everything after it. `k = 40` is a common default.

```rust
fn sample(logits: &[f32], temperature: f32, top_k: usize) -> u32 {
    let mut scaled: Vec<(usize, f32)> = logits.iter().enumerate()
        .map(|(i, &v)| (i, v / temperature.max(1e-6)))
        .collect();
    scaled.sort_by(|a, b| b.1.total_cmp(&a.1));
    scaled.truncate(top_k.max(1));
    ...
    for (&(id, _), &w) in scaled.iter().zip(&weights) {
        threshold -= w;
        if threshold <= 0.0 { return id as u32; }
    }
    scaled[0].0 as u32
}
```

Subtracting `max` before `exp` is not a nicety. Logits reach 30 or more, and
`exp(30)` overflows into `inf`, at which point every weight is `NaN`. Chapter
13.2, arriving from a different direction.
...
prompt: "the borrow checker"
  ...
  T=1.5 k=27  "the borrow checker is not your it ellyoue anet em couldeeeeue be a ant aodensasat ykend aght ogsab"
```
...
Notice too that `T=0.2` and `T=0.8` produce identical text. When the model is
this certain, the probability mass is a single spike and temperature has almost
nothing to flatten. Temperature only matters when the model is genuinely
uncertain.
...
| Generation repeats one token forever | Greedy sampling. Add temperature and top-k |
| Generation is fluent nonsense | Undertrained. This is what a small model on little data does |
| Generation is `NaN` or one id | Overflow in your softmax — subtract the max |
...
- Temperature and top-k are what stop generation looping or derailing.
